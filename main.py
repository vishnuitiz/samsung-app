...
def create_table_memory_management_events_tracing():
    try:
        ...
        CREATE TABLE IF NOT EXISTS memory_management_events_tracing (
            ...
            proces_name VARCHAR(255),
            total_memory_gb FLOAT,
            available_memory_gb FLOAT, 
            used_memory_gb FLOAT,
            memory_percent_used FLOAT,
            total_swap_gb FLOAT,
            total_swap_gb_1 FLOAT,
            swap__percent_used FLOAT,
            recorded_at DATETIME
        )"""
        db_cursor.execute(create_query)
        db_connection.commit()
...
def generate_line_graph(x_data, y_data, x_column, y_column):
    fig, ax = plt.subplots()
...
def create_dedicated_page_l3_cache_stats(label, table):
    def dedicated_page(page):
        def on_time_stamp_click(e):
            ...
            garph_data = fetch_column_data(table, "hit_rate")
            image_base64 = generate_line_graph(tid_data,garph_data,"tid", "hit_rate")  #generate_line_graph(tid_data,"tid")
        ...
        def on_pid_click(e):
            ...
            garph_data = fetch_column_data(table, "hit_rate")
            image_base64 = generate_line_graph(pid_data,garph_data,"pid", "hit_rate")
        ...
        generate_button = ft.ElevatedButton(text="Generate Stats", on_click=lambda e: insert_dummy_data_l3_cache_stats(),
                                            width=200, height=50)
...
def create_dedicated_page_paging_operations(label, table):
    def dedicated_page(page):
        def on_time_stamp_click(e):
            ...
            tid_data = fetch_column_data(table, "recorded_at")
            print("Timestamp data fetched:", tid_data)
            garph_data = fetch_column_data(table, "page_faults")
            image_base64 = generate_line_graph(tid_data, garph_data,"recorded_at","page_faults") #generate_line_graph(tid_data, "recorded_at")
            display_data(image_base64, tid_data, "Timestamp Data")
        def on_pid_click(e):
            # Adjust this function to fetch data from the paging_operations table
            pid_data = fetch_column_data(table, "pid")
            print("PID data fetched:", pid_data)
            garph_data = fetch_column_data(table, "page_faults")
            image_base64 = generate_line_graph(pid_data, garph_data,"pid","page_faults")
            display_data(image_base64, pid_data, "PID Data")
        def display_data(image_base64, table_data, title):
            page.controls.clear()
            image = ft.Image(src_base64=image_base64, width=600, height=400)
        ...
        generate_button = ft.ElevatedButton(text="Generate Stats", on_click=lambda e: insert_dummy_data_paging_operations(),
                                            width=200, height=50)
        time_stamp_button = ft.ElevatedButton(text="Time Stamp", on_click=on_time_stamp_click, width=200, height=50)
        pid_button = ft.ElevatedButton(text="PID", on_click=on_pid_click, width=200, height=50)
...
def create_dedicated_page_swap_io_overhead(label, table):
    def dedicated_page(page):
        def on_time_stamp_click(e):
            ...
            tid_data = fetch_column_data(table, "recorded_at")
            print("Timestamp data fetched:", tid_data)
            garph_data = fetch_column_data(table, "write_mb_s")
            image_base64 =  generate_line_graph(tid_data,garph_data,"recorded_at","write_mb_s") #generate_line_graph(tid_data, "recorded_at")
            display_data(image_base64, tid_data, "TID Data")
        def on_pid_click(e):
            # Adjust this function to fetch data from the paging_operations table
            pid_data = fetch_column_data(table, "pid")
            print("PID data fetched:", pid_data)
            garph_data = fetch_column_data(table, "write_mb_s")
            image_base64 =  generate_line_graph(pid_data,garph_data,"pid","write_mb_s")
            display_data(image_base64, pid_data, "PID Data")
        ...
        generate_button = ft.ElevatedButton(text="Generate Stats", on_click=lambda e: insert_dummy_data_swap_io_overhead(),
                                            width=200, height=50)
        time_stamp_button = ft.ElevatedButton(text="Time Stamp", on_click=on_time_stamp_click, width=200, height=50)
        pid_button = ft.ElevatedButton(text="PID", on_click=on_pid_click, width=200, height=50)
...
def create_dedicated_page_user_space_memory_alloc_and_dealloc(label, table):
    def dedicated_page(page):
        def on_time_stamp_click(e):
            ...
            # Adjust this function to fetch data from the paging_operations table
            print("Timestamp data fetched:", tid_data)
            garph_data = fetch_column_data(table, "count")
            image_base64 =  generate_line_graph(tid_data,garph_data, "recorded_at", "count") #generate_line_graph(tid_data, "recorded_at")
            display_data(image_base64, tid_data, "TID Data")
        def on_pid_click(e):
            # Adjust this function to fetch data from the paging_operations table
            pid_data = fetch_column_data(table, "pid")
            print("PID data fetched:", pid_data)
            garph_data = fetch_column_data(table, "count")
            image_base64 =  generate_line_graph(pid_data,garph_data, "pid", "count")
            display_data(image_base64, pid_data, "PID Data")
        def display_data(image_base64, table_data, title):
            page.controls.clear()
            image = ft.Image(src_base64=image_base64, width=600, height=400)
        ...
        generate_button = ft.ElevatedButton(text="Generate Stats", on_click=lambda e: insert_dummy_data_user_space_memory_alloc_and_dealloc(),
                                            width=200, height=50)
        time_stamp_button = ft.ElevatedButton(text="Time Stamp", on_click=on_time_stamp_click, width=200, height=50)
        pid_button = ft.ElevatedButton(text="PID", on_click=on_pid_click, width=200, height=50)
...
def create_dedicated_page_vm_stats(label, table):
    def dedicated_page(page):
        def on_time_stamp_click(e):
            ...
            tid_data = fetch_column_data(table, "recorded_at")
            print("Timestamp data fetched:", tid_data)
            garph_data = fetch_column_data(table, "pss")
            image_base64 = generate_line_graph(tid_data,garph_data,"recorded_at","pss") #generate_line_graph(tid_data, "recorded_at")
            display_data(image_base64, tid_data, "TID Data")
        ...
        def on_pid_click(e):
            ...
            print("PID data fetched:", pid_data)
            garph_data = fetch_column_data(table, "pss")
            image_base64 = generate_line_graph(pid_data,garph_data,"pid","pss")
            display_data(image_base64, pid_data, "PID Data")
        ...
        generate_button = ft.ElevatedButton(text="Generate Stats", on_click=lambda e: insert_dummy_data_vm_stats(),
                                            width=200, height=50)
        time_stamp_button = ft.ElevatedButton(text="Time Stamp", on_click=on_time_stamp_click, width=200, height=50)
        pid_button = ft.ElevatedButton(text="PID", on_click=on_pid_click, width=200, height=50)
...
def create_dedicated_page_memory_management_events_tracing(label, table):
    def dedicated_page(page):
        def on_time_stamp_click(e):
            ...
            tid_data = fetch_column_data(table, "recorded_at")
            print("Timestamp data fetched:", tid_data)
            garph_data = fetch_column_data(table, "memory_percent_used")
            image_base64 =  generate_line_graph(tid_data,garph_data,"recorded_at","memory_percent_used") #generate_line_graph(tid_data, "recorded_at")
            display_data(image_base64, tid_data, "TID Data")
        ...
        def on_pid_click(e):
            ...
            print("PID data fetched:", pid_data)
            garph_data = fetch_column_data(table, "memory_percent_used")
            image_base64 =  generate_line_graph(pid_data,garph_data,"pid","memory_percent_used")
            display_data(image_base64, pid_data, "PID Data")
        ...
        generate_button = ft.ElevatedButton(text="Generate Stats", on_click=lambda e: insert_dummy_data_memory_management_events_tracing(),
                                            width=200, height=50)
        time_stamp_button = ft.ElevatedButton(text="Time Stamp", on_click=on_time_stamp_click, width=200, height=50)
        pid_button = ft.ElevatedButton(text="PID", on_click=on_pid_click, width=200, height=50)
...
def create_dedicated_page_psi_stats_resource_pressure(label, table):
    def dedicated_page(page):
        def on_time_stamp_click(e):
            ...
            tid_data = fetch_column_data(table, "recorded_at")
            print("Timestamp data fetched:", tid_data)
            garph_data = fetch_column_data(table, "total")
            image_base64 = generate_line_graph(tid_data,garph_data,"recorded_at","total") #generate_line_graph(tid_data, "recorded_at")
            display_data(image_base64, tid_data, "TID Data")
        ...
        def on_pid_click(e):
            ...
            print("PID data fetched:", pid_data)
            garph_data = fetch_column_data(table, "total")
            image_base64 = generate_line_graph(pid_data,garph_data,"pid","total")
            display_data(image_base64, pid_data, "PID Data")
        ...
        generate_button = ft.ElevatedButton(text="Generate Stats", on_click=lambda e: insert_dummy_data_psi_stats_resource_pressure(),
                                            width=200, height=50)
        time_stamp_button = ft.ElevatedButton(text="Time Stamp", on_click=on_time_stamp_click, width=200, height=50)
        pid_button = ft.ElevatedButton(text="PID", on_click=on_pid_click, width=200, height=50)
